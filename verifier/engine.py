...
class VerificationEngine:
    ...
    def verify(self, original_code: str, patched_code: str,
        ...
        if self.config.run_tests:
            if is_c:
                regression_result = self._run_c_regression_check(original_code, patched_code)
            else:
        ...
        if self.config.check_behavior:
            if is_c:
                behavior_result = self._validate_c_behavior(original_code, patched_code)
            else:
    ...
    def _run_c_regression_check(self, original_code: str, patched_code: str) -> Dict:
        """
        Structural regression check for C: confirms every function defined
        in the original still exists (by name) in the patched code.

        Deliberately does not compile-and-run the patch as a smoke test the
        way the Python path does — executing arbitrary AI-generated C as a
        "regression test" would itself be a real safety risk (unlike Python
        code run in-process, a C binary can corrupt memory or crash the
        host). This is weaker than the Python check as a result: it catches
        a patch that silently deleted a function, not one that kept a
        function's signature but broke its behavior.
        """
        original_funcs = {name for name, *_ in self.rewind.split_c_functions(original_code)}
        patched_funcs = {name for name, *_ in self.rewind.split_c_functions(patched_code)}
        missing = sorted(original_funcs - patched_funcs)

        return {
            "passed": not missing,
            "tested": True,
            "original_functions": sorted(original_funcs),
            "patched_functions": sorted(patched_funcs),
            "missing_functions": missing,
        }

    def _validate_c_behavior(self, original_code: str, patched_code: str) -> Dict:
        """
        Structural behavior-preservation check for C: same function set, and
        no function's body shrank drastically — a crude proxy for "the fix
        didn't just gut the function to make the finding disappear,"
        mirroring what the Python AST-diff check targets for that language.
        """
        original_funcs = {name: body for name, _sig, _start, body in self.rewind.split_c_functions(original_code)}
        patched_funcs = {name: body for name, _sig, _start, body in self.rewind.split_c_functions(patched_code)}

        changes = []
        for name, original_body in original_funcs.items():
            if name not in patched_funcs:
                changes.append(f"Removed: {name}")
                continue
            original_lines = original_body.count('\n') + 1
            patched_lines = patched_funcs[name].count('\n') + 1
            if patched_lines < max(2, original_lines * 0.3):
                changes.append(f"Drastically shrunk: {name} ({original_lines} -> {patched_lines} lines)")

        return {
            "preserved": len(changes) <= 2,
            "tested": True,
            "changes": changes,
        }
